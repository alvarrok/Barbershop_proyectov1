...
import { 
  ...
  SimpleGrid, Select, FileInput, Notification
} from '@mantine/core';
...
export default function AdminDashboard() {
  ...
  // --- LÓGICA DE FOTOS (SUBIR DESDE PC) ---
  const handleImageUpload = (file) => {
      if (!file) return;
      const reader = new FileReader();
      reader.onloadend = () => {
          // Esto convierte la imagen a texto Base64 para guardarla en la BD
          setFormBarber({ ...formBarber, imagenUrl: reader.result });
      };
  ...
  const handleSaveBarber = async () => {
      ...
      try {
          ...
          fetchData();
      } catch (e) { notifications.show({ message: 'Error al guardar. Revisa el DNI o el tamaño de la foto.', color: 'red' }); }
  ...
  const recentNotifications = appointments.slice(0, 10).map(a => ({
      id: a.id,
      title: a.estado === 'PENDIENTE' ? 'Nueva Reserva' : 'Cita Actualizada',
      msg: `${a.clienteNombre} - ${dayjs(a.fechaInicio).format('DD/MM HH:mm')}`,
      color: a.estado === 'PENDIENTE' ? 'yellow' : 'blue',
      time: dayjs(a.createdAt || a.fechaInicio).fromNow()
  }));
      ...
      <AppShell.Main>
        <Tabs defaultValue="agenda" variant="pills" color="yellow" radius="md">
            <Tabs.List mb="lg" style={{background:'#111', padding:'10px'}}>
                ...
                <Tabs.Tab value="notis" leftSection={<IconBell size={18}/>} c="white">Notificaciones</Tabs.Tab>
            </Tabs.List>
                ...
                <Grid>
                    <Grid.Col span={{base:12, md:4}}>
                        <Card withBorder radius="md" p="md" style={{background:'#111', borderColor:'#333', position:'sticky', top:'20px'}}>
                            ...
                            {/* CAMPO DE URL O SUBIDA DE ARCHIVO */}
                            <Tabs defaultValue="url" variant="outline" mb="md">
                                <Tabs.List>
                                    <Tabs.Tab value="url" c="dimmed">Enlace URL</Tabs.Tab>
                                    <Tabs.Tab value="file" c="dimmed">Subir PC</Tabs.Tab>
                                </Tabs.List>
                                <Tabs.Panel value="url" pt="xs">
                                    <TextInput placeholder="https://..." value={formBarber.imagenUrl?.startsWith('data') ? '' : formBarber.imagenUrl} onChange={(e) => setFormBarber({...formBarber, imagenUrl: e.target.value})} styles={{input:{background:'#222', color:'white'}}} rightSection={<IconPhoto size={16} color="gray"/>} />
                                </Tabs.Panel>
                                <Tabs.Panel value="file" pt="xs">
                                    <FileInput placeholder="Seleccionar foto" accept="image/*" onChange={handleImageUpload} styles={{input:{background:'#222', color:'white'}}} leftSection={<IconUpload size={16}/>} />
                                </Tabs.Panel>
                            </Tabs>
            ...
            <Tabs.Panel value="notis">
                <Card withBorder radius="md" p="md" style={{background:'#111', borderColor:'#333'}}>
                    <Text fw={700} c="white" mb="lg">Última Actividad</Text>
                    <ScrollArea h={500}>
                        {recentNotifications.length > 0 ? recentNotifications.map(n => (
                            <Notification key={n.id} icon={n.color==='yellow'?<IconClock size={18}/>:<IconCheck size={18}/>} color={n.color} title={n.title} withCloseButton={false} style={{backgroundColor:'#1a1a1a', marginBottom:'10px', border:'1px solid #333'}}>
                                <Text size="sm" c="white">{n.msg}</Text>
                                <Text size="xs" c="dimmed">{n.time}</Text>
                            </Notification>
                        )) : <Text c="dimmed" align="center">Sin actividad reciente.</Text>}
                    </ScrollArea>
                </Card>
            </Tabs.Panel>
        </Tabs>
