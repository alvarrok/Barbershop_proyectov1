...
// FIXED: Added IconArrowRight to imports
import { 
  ...
  IconUsers, IconUserOff, IconUserCheck, IconGenderMale, IconGenderFemale, IconArrowRight
...
export default function AdminDashboard() {
  ...
  const handleSaveBarber = async () => {
      if(!formBarber.nombre || !formBarber.dni) return notifications.show({message:'Name and ID required', color:'red'});
      setLoadingAction(true);
      try {
          if(isEditingBarber) {
              await api.put(`/barbers/${formBarber.id}`, formBarber);
              notifications.show({ message: 'Barber updated', color: 'green' });
          } else {
              await api.post('/barbers', formBarber);
              notifications.show({ message: 'Barber registered', color: 'green' });
          }
          setFormBarber(initialBarberForm);
          setIsEditingBarber(false);
          fetchData();
      } catch (e) { notifications.show({ message: 'Error saving (check data)', color: 'red' }); }
  ...
  const toggleBarberStatus = async (b) => {
      try {
          ...
          fetchData();
      } catch(e) { notifications.show({ message: 'Error changing status', color: 'red' }); }
  };

  // --- SERVICE ACTIONS ---
  const handleSaveService = async () => {
    if(!formService.nombre) return notifications.show({message:'Name missing', color:'red'});
    setLoadingAction(true);
    try {
  ...
  const confirmDelete = async () => {
      ...
      try {
        ...
        fetchData(); notifications.show({ message: 'Deleted', color: 'green' });
      } catch(e) { notifications.show({ message: 'Error (Might have associated appointments)', color: 'red' }); }
      setLoadingAction(false);
  ...
  const sendWhatsAppInternal = async (appt, type) => {
      if (waStatus !== 'READY') { setShowQRModal(true); return notifications.show({ message: 'Connect WhatsApp', color: 'red' }); }
      ...
      let msg = type==='avisar' ? `Hola ${name}, reminder of your appointment today at ${time}.` : `Hola ${name}, your appointment has been cancelled.`;
      
      notifications.show({ id: 'wa', loading: true, message: 'Sending...' });
      try { await api.post('/send-whatsapp', { phone, message: msg }); notifications.update({ id: 'wa', color: 'green', message: 'Sent', loading: false }); } 
      catch (e) { notifications.update({ id: 'wa', color: 'red', message: 'Error', loading: false }); }
  };

  const handleConfirmCut = async () => {
      try { await api.put(`/appointments/${selectedAppt.id}`, { estado: 'COMPLETADO' }); notifications.show({ message: 'Paid', color: 'blue', icon: <IconCheck/> }); fetchData(); setSelectedAppt(null); } catch(e){}
  };
    ...
    <AppShell header={{ height: 70 }} padding="md" styles={{ main: { background: '#0a0a0a', color: 'white' } }}>
      <AppShell.Header style={{background: '#111', borderBottom: '1px solid #c49b63', padding: '0 20px', display:'flex', alignItems:'center'}}>
         <Group justify="space-between" w="100%">
             ...
             <Group>
                 <Indicator color={waStatus === 'READY' ? 'green' : 'red'} processing={waStatus==='QR_READY'} size={12}><Button variant="subtle" color="gray" leftSection={<IconBrandWhatsapp/>} onClick={() => setShowQRModal(true)}>{waStatus === 'READY' ? 'Connected' : 'Connect'}</Button></Indicator>
                 <Button variant="subtle" color="yellow" onClick={() => {localStorage.removeItem('adminToken'); navigate('/');}}>Exit</Button>
             </Group>
         </Group>
      </AppShell.Header>
      ...
      <AppShell.Main>
        <Tabs defaultValue="agenda" variant="pills" color="yellow" radius="md">
            <Tabs.List mb="lg" style={{background:'#111', padding:'10px'}}>
                ...
                <Tabs.Tab value="finance" leftSection={<IconCurrencyDollar size={18}/>} c="white">Finances</Tabs.Tab>
                <Tabs.Tab value="services" leftSection={<IconScissors size={18}/>} c="white">Services</Tabs.Tab>
                <Tabs.Tab value="team" leftSection={<IconUsers size={18}/>} c="white">Team</Tabs.Tab>
            </Tabs.List>
            ...
            <Tabs.Panel value="agenda">
                <Grid>
                    ...
                    <Grid.Col span={{ base: 12, md: 8 }}><Card withBorder radius="md" p="0" style={{background:'#111', borderColor:'#333'}}><div style={{padding:'20px', background:'#1a1a1a', borderBottom:'1px solid #333'}}><Text fw={700} size="xl" c="#c49b63">{dayjs(selectedDate).format('dddd D [of] MMMM')}</Text></div>{renderSchedule()}</Card></Grid.Col>
                </Grid>
            </Tabs.Panel>

            {/* FINANCES (No Chart to avoid error) */}
            <Tabs.Panel value="finance">
                <Grid>
                    <Grid.Col span={12}><Card withBorder radius="md" p="lg" style={{background:'#111', borderColor:'#333'}}><Group><DatePickerInput label="From" value={finStartDate} onChange={setFinStartDate} styles={{input:{background:'#222', color:'white'}, label:{color:'white'}}} /><IconArrowRight color="gray" style={{marginTop:'25px'}} /><DatePickerInput label="To" value={finEndDate} onChange={setFinEndDate} styles={{input:{background:'#222', color:'white'}, label:{color:'white'}}} /><Card p="xs" radius="sm" style={{background:'#1a472a', marginLeft:'auto', minWidth:'200px'}}><Text size="xs" c="white">REALIZED PROFIT</Text><Text size="xl" fw={900} c="white">S/. {finTotal.toFixed(2)}</Text></Card></Group></Card></Grid.Col>
            ...
            <Tabs.Panel value="services">
                 <Card withBorder radius="md" p="lg" style={{background:'#111', borderColor:'#333'}}>
                    <Group align="flex-end" mb="lg">
                        <TextInput label="Name" value={formService.nombre} onChange={(e)=>setFormService({...formService, nombre: e.target.value})} styles={{input:{background:'#333', color:'white'}, label:{color:'white'}}}/>
                        <NumberInput label="Minutes" value={formService.minutos} onChange={(val)=>setFormService({...formService, minutos: val})} styles={{input:{background:'#333', color:'white'}, label:{color:'white'}}}/>
                        <NumberInput label="Price" value={formService.precio} onChange={(val)=>setFormService({...formService, precio: val})} styles={{input:{background:'#333', color:'white'}, label:{color:'white'}}}/>
                        <Button color={isEditingService?"blue":"yellow"} loading={loadingAction} onClick={handleSaveService} styles={{root:{color:'black'}}}>{isEditingService?"SAVE":"ADD"}</Button>
                        {isEditingService && <Button variant="default" onClick={()=>{setIsEditingService(false);setFormService({id:null,nombre:'',minutos:30,precio:0})}}>Cancel</Button>}
                    </Group>
                    <Table>
                        <Table.Thead><Table.Tr><Table.Th c="dimmed">Name</Table.Th><Table.Th c="dimmed">Duration</Table.Th><Table.Th c="dimmed">Price</Table.Th><Table.Th>Actions</Table.Th></Table.Tr></Table.Thead>
                ...
                <Grid>
                    <Grid.Col span={{base:12, md:4}}>
                        <Card withBorder radius="md" p="md" style={{background:'#111', borderColor:'#333', position:'sticky', top:'20px'}}>
                            <Text fw={700} c="white" mb="md" tt="uppercase" style={{borderBottom:'2px solid #c49b63', display:'inline-block'}}>{isEditingBarber ? 'Edit Barber' : 'New Barber'}</Text>
                            <Center mb="md" style={{flexDirection:'column'}}>
                                ...
                                <Text size="xs" c="dimmed" mt="xs">Preview</Text>
                            </Center>
                            <TextInput label="Full Name *" placeholder="Ex. John Doe" mb="xs" value={formBarber.nombre} onChange={(e) => setFormBarber({...formBarber, nombre: e.target.value})} styles={{input:{background:'#222', color:'white'}, label:{color:'white'}}} leftSection={<IconUser size={16}/>} />
                            <Grid gutter="xs">
                                <Grid.Col span={6}><TextInput label="ID *" placeholder="8 digits" mb="xs" value={formBarber.dni} onChange={(e) => setFormBarber({...formBarber, dni: e.target.value.replace(/\D/g, '').slice(0,8)})} styles={{input:{background:'#222', color:'white'}, label:{color:'white'}}} leftSection={<IconId size={16}/>}/></Grid.Col>
                                <Grid.Col span={6}><TextInput label="Phone" placeholder="999..." mb="xs" value={formBarber.telefono} onChange={(e) => setFormBarber({...formBarber, telefono: e.target.value})} styles={{input:{background:'#222', color:'white'}, label:{color:'white'}}} leftSection={<IconPhone size={16}/>}/></Grid.Col>
                            </Grid>
                            <Select label="Sex" mb="xs" data={['Masculino', 'Femenino', 'Otro']} value={formBarber.sexo} onChange={(val) => setFormBarber({...formBarber, sexo: val})} styles={{input:{background:'#222', color:'white'}, label:{color:'white'}, dropdown:{background:'#222', color:'white'}}}/>
                            <TextInput label="Photo URL" placeholder="https://..." mb="lg" value={formBarber.imagenUrl} onChange={(e) => setFormBarber({...formBarber, imagenUrl: e.target.value})} styles={{input:{background:'#222', color:'white'}, label:{color:'white'}}} rightSection={<IconPhoto size={16} color="gray"/>} />
                            <Group grow><Button color={isEditingBarber?"blue":"yellow"} loading={loadingAction} onClick={handleSaveBarber} styles={{root:{color: isEditingBarber?'white':'black'}}}>{isEditingBarber ? "SAVE" : "REGISTER"}</Button>{isEditingBarber && <Button variant="default" onClick={()=>{setFormBarber(initialBarberForm); setIsEditingBarber(false)}}>Cancel</Button>}</Group>
                        </Card>
                    </Grid.Col>
                            ...
                            {barbers.map(b => (
                                <Card key={b.id} withBorder radius="lg" p="0" style={{background:'#1a1a1a', borderColor: b.activo ? '#333' : '#500000', opacity: b.activo ? 1 : 0.7, transition:'all 0.3s'}}>
                                    ...
                                    <Box p="md" ta="center">
                                        ...
                                        <Badge variant={b.activo ? "filled" : "outline"} color={b.activo ? "green" : "gray"} mb="md">{b.activo ? 'ACTIVE' : 'INACTIVE'}</Badge>
        ...
        <Modal opened={!!selectedAppt} onClose={() => setSelectedAppt(null)} title="Manage Appointment" centered styles={{header:{background:'#222', color:'white'}, body:{background:'#222', color:'white'}}}>{selectedAppt && (<div style={{display:'flex', flexDirection:'column', gap:'15px'}}><Group justify="space-between"><div><Text size="lg" fw={700} c="white">{selectedAppt.clienteNombre}</Text><Text size="sm" c="yellow">{selectedAppt.service?.nombre}</Text><Text size="xs" c="dimmed">Barber: {selectedAppt.barber?.nombre || 'Any'}</Text></div><Badge color={selectedAppt.estado==='COMPLETADO'?'blue':'yellow'}>{selectedAppt.estado}</Badge></Group><Card withBorder style={{background:'#1a1a1a', borderColor:'#333', padding:'10px'}}><Group mb={5}><IconId size={16} color="gray"/><Text size="sm" c="dimmed">ID: <span style={{color:'white'}}>{selectedAppt.clienteDni}</span></Text></Group><Group mb={5}><IconPhone size={16} color="gray"/><Text size="sm" c="dimmed">Tel: <span style={{color:'white'}}>{selectedAppt.clientePhone}</span></Text></Group><Group><IconClock size={16} color="gray"/><Text size="sm" c="dimmed">Date: <span style={{color:'white'}}>{dayjs(selectedAppt.fechaInicio).format('DD/MM/YYYY hh:mm A')}</span></Text></Group></Card>{selectedAppt.estado !== 'COMPLETADO' && selectedAppt.estado !== 'CANCELADO' && (<Button leftSection={<IconCheck size={20}/>} color="blue" fullWidth onClick={handleConfirmCut}>Confirm & Pay (S/.{selectedAppt.service?.precio})</Button>)}<Button leftSection={<IconMessage size={18}/>} color="green" variant="light" fullWidth onClick={() => sendWhatsAppInternal(selectedAppt, 'avisar')}>Notify Client</Button><Button color="red" variant="subtle" fullWidth onClick={async () => { if(window.confirm('Cancel?')) { await api.put(`/appointments/${selectedAppt.id}/cancel`); sendWhatsAppInternal(selectedAppt, 'cancel'); fetchData(); setSelectedAppt(null); } }}>Cancel Appointment</Button></div>)}</Modal>
        <Modal opened={deleteModalOpen} onClose={() => setDeleteModalOpen(false)} title="Delete?" centered styles={{header:{background:'#222', color:'white'}, body:{background:'#222', color:'white'}}}><Text c="dimmed" size="sm" mb="lg">Irreversible action.</Text><Group justify="flex-end"><Button variant="default" onClick={() => setDeleteModalOpen(false)}>Cancel</Button><Button color="red" loading={loadingAction} onClick={confirmDelete}>Delete</Button></Group></Modal>
      </AppShell.Main>
    </AppShell>
  );
}